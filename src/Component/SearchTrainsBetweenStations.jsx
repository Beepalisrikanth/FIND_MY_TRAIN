...
const SearchTrainsBetweenStations = () => {
    ...
    // console.log(from,to)
    console.log(ftData?.data?.totalTrains)
    ...
    const submit = () => {
        if (from && to) {
            ...
            alert(` ${from} ${to} enter a valid inputs`)
        }
    }
    ...
    return (
        <div className=' bg-white rounded-[20px] shadow-lg p-1 w-[100%] '>
            <form onSubmit={(e) => { e.preventDefault(); STBS() }} className='px-5 flex flex-col m-4  '>
                ...
                <button type='submit' className='text-[1.5em] bg-blue-600 text-white w-[100%] py-2 rounded-[20px] hover:bg-blue-700 transition mb-4' onClick={submit}>Search</button>
            </form>
            <div>
                {/* {ftData?.data?.totalTrains} */}
            </div>
        </div>
    )
}
